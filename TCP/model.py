...
class TCP(nn.Module):
	...
	def forward(self, img, state, target_point):
		...
		for _ in range(self.config.pred_len):
			...
			wp_att = self.wp_att(torch.cat([h, traj_hidden_state[:, _]], 1)).view(-1, 1, 8, 10)
	...
	def process_action(self, pred, command, speed, target_point):
		action = self.get_action_beta_torch(pred['mu_branches'].view(1,2), pred['sigma_branches'].view(1,2))
		acc, steer = action.detach().numpy()[0]
		#acc, steer = action.detach().numpy()[0].astype(np.float64)
		# mu_branches: shape (1,2)  e.g. tensor([[0.6921, 0.6977]])
		...
		#}
		return steer, throttle, brake, None
	
	def get_action_beta_torch(self,alpha, beta):
		"""
		alpha,beta: shape (B,2)  -> [acc, steer] için α/β
		döner:      action: shape (B,2) in [-1,1] -> [acc, steer]
		"""
		x = torch.zeros_like(alpha)           # default 0
		x[:, 1] += 0.5                        # steer için 0.5 taban (orijinal koddaki gibi)

		mask1 = (alpha > 1) & (beta > 1)      # mode
		x[mask1] = (alpha[mask1]-1) / (alpha[mask1] + beta[mask1] - 2).clamp(min=1e-8)

		mask2 = (alpha <= 1) & (beta > 1)     # 0.0
		x[mask2] = 0.0

		mask3 = (alpha > 1) & (beta <= 1)     # 1.0
		x[mask3] = 1.0

		mask4 = (alpha <= 1) & (beta <= 1)    # mean
		denom = (alpha[mask4] + beta[mask4]).clamp(min=1e-8)
		x[mask4] = alpha[mask4] / denom
	...
	def control_pid(self, waypoints, velocity, target, state):
		...
		#target = target[0].squeeze().detach().cpu().numpy()
